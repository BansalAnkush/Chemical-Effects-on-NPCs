...
data, labels = load_data(TRAIN_DATA_FOLDER)
...
def run_model(args):
    ...
    X_train_val, X_test, y_train_val, y_test = train_test_split(X, y, test_size=0.1, random_state=42 + iteration)
    ...
    rf = RandomForestClassifier(random_state=42 + iteration)
    param_grid = {
...
if __name__ == '__main__':
    ...
    with multiprocessing.Pool() as pool:
        results = list(tqdm(pool.imap(run_model, [(i, data, labels) for i in range(n_iterations)]), total=n_iterations, desc="Running iterations"))
    ...
    def plot_metrics(val_metrics, test_metrics, title):
        ...
        print("Visualization saved as 'random_forest_performance_metrics.png'")
        plt.close()
    ...
    plot_metrics(val_metrics, test_metrics, 'Random Forest Model Performance Metrics')
    ...
    metrics_df.to_csv('random_forest_performance_metrics.csv', index=False)
    print("Performance metrics saved as 'random_forest_performance_metrics.csv'")
