...
def run_model(iteration):
    ...
    svm = SVC(random_state=42 + iteration)
    
    halving_random_search = HalvingRandomSearchCV(
        estimator=svm,
        param_distributions=param_dist,
        n_candidates=10,  # Start with 10 parameter settings
        cv=5,
        factor=2,
        random_state=42 + iteration,
        n_jobs=1  # Use 1 job per iteration as we're already using multiprocessing
    )
...
if __name__ == '__main__':
    ...
    def plot_metrics(val_metrics, test_metrics, title):
        ...
        print("Visualization saved as 'svm_performance_metrics.png'")
        plt.close()
    ...
    plot_metrics(val_metrics, test_metrics, 'SVM Model Performance Metrics')
    ...
    print("Performance metrics saved as 'svm_performance_metrics.csv'")
